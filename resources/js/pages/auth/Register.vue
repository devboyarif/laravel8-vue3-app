        ...
        <form class="card card-md" @submit.prevent="userRegistration" autocomplete="off">
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="Enter name" v-model="registerUserForm.name" :class="{ 'is-invalid': registerUserForm.errors.has('name') }">
                    <has-error :form="registerUserForm" field="name"></has-error>
                </div>
                <div class="mb-3">
                    <label class="form-label">Email address</label>
                    <input type="email" class="form-control" placeholder="Enter email" v-model="registerUserForm.email" :class="{ 'is-invalid': registerUserForm.errors.has('email') }">
                    <has-error :form="registerUserForm" field="email"></has-error>
                </div>
                <div class="mb-3">
                    <label class="form-label">Password</label>
                    <div class="input-group input-group-flat">
                        <input :type="visibility ? 'text' : 'password'" class="form-control" placeholder="Password" autocomplete="off" v-model="registerUserForm.password" :class="{ 'is-invalid': registerUserForm.errors.has('password') }">
                        <span class="input-group-text">
                            <a @click.prevent="visibility = !visibility" href="javascript:void(0)" class="link-secondary" title="Show password" data-bs-toggle="tooltip">
                                <svg v-if="visibility" xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                ...
                <div class="form-footer">
                    <button type="submit" :disabled="registerUserForm.busy"  class="btn btn-primary w-100">Create new account</button>
                </div>
            </div>
        </form>
